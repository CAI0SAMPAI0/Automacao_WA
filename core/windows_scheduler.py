...
def _get_app_path() -> Path:
    """Retorna o path base do app — sempre calculado na hora, nunca em importação."""
    return Path(get_app_base_dir()).absolute()
...
def create_windows_task(task_id, task_name, schedule_time, schedule_date=None,
    ...
    try:
        ...
        print(f"[OK] Tarefa {task_full_name} criada com sucesso")
        if daily:
            modo = "todos os dias" if include_weekends else "dias uteis (Seg-Sex)"
            print(f"[INFO] Agendada para: {schedule_time} | Recorrencia: {modo}")
        else:
            print(f"[INFO] Agendada para: {schedule_date} as {schedule_time}")
...
def delete_windows_task(task_id):
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print(f"[AVISO] Nao foi possivel remover {task_name}: {result.stderr}")
